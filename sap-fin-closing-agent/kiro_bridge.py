"""Kiro stdio bridge for SAP Financial Closing Agent — auto-discovers tools from AgentCore."""
...
OKTA_REDIRECT_URI = os.environ.get("OKTA_REDIRECT_URI", "http://localhost:8089/callback")
OKTA_CALLBACK_PORT = int(OKTA_REDIRECT_URI.split(":")[-1].split("/")[0])
SSM_PARAM = os.environ.get("AGENTCORE_ARN_SSM_PARAM", "/sap_generated/sap_fin_closing_agent/agent_arn")
...
# Auto-discover tools from AgentCore
_cached_tools = None

async def _fetch_tools_from_agentcore():
    global _cached_tools
    if _cached_tools: return _cached_tools
    ...
    async with streamablehttp_client(mcp_url, headers, timeout=timedelta(seconds=60), terminate_on_close=False) as (r, w, _):
        async with ClientSession(r, w) as session:
            ...
            _cached_tools = result.tools
            return _cached_tools

server = Server("sap-fin-closing-bridge")

@server.list_tools()
async def list_tools():
    try:
        tools = await _fetch_tools_from_agentcore()
        return [types.Tool(name=t.name, description=t.description, inputSchema=t.inputSchema) for t in tools]
    except Exception as e:
        return [types.Tool(name="error", description=f"Failed to fetch tools: {e}",
