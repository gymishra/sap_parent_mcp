...
mcp = FastMCP("AI Factory MCP Server", host="0.0.0.0", stateless_http=True)
...
def _get_token(ctx: Context = None) -> str:
    ...
    # 2. Env var fallback (set by _run_strands before calling tools)
...
_SERVER_TEMPLATE = '''"""
{description}
Auto-generated by AI Factory MCP Server.
"""
...
def _get_token(ctx: Context, bearer_token: str = ""):
    # 1. Explicit bearer_token parameter (passed by bridge)
    if bearer_token:
        return bearer_token
    # 2. Custom AgentCore header
    try:
        req = ctx.request_context.request
        for header_name in ["x-amzn-bedrock-agentcore-runtime-custom-saptoken",
                            "X-Amzn-Bedrock-AgentCore-Runtime-Custom-SapToken",
...
def _discover_relevant_services(token: str, domain_keywords: list) -> list:
    """Query SAP catalog and filter services matching domain keywords. Uses in-memory cache if available."""
    try:
        # Use cached catalog if available — no SAP call needed
        if _catalog_cache:
            logger.info(f"Using cached catalog ({len(_catalog_cache)} services) for keyword search: {domain_keywords}")
            services = list(_catalog_cache.values())
        else:
            data = _sap_get(CATALOG_URL, token, {"$format": "json"})
...
def _get_entities_for_service(service_path: str, token: str) -> list:
    """Get top entity types for a service. Uses in-memory cache if available."""
    # Check entity cache first — keyed by service title (last segment of path)
    svc_title = service_path.rstrip("/").split("/")[-1]
    if svc_title in _entity_cache:
        cached = _entity_cache[svc_title]
        entities = [e.get("entity_type", e.get("entity_set", "")) for e in cached
                    if not e.get("entity_type", "").startswith(("I_", "SAP__"))]
        if entities:
            logger.info(f"Entity cache hit for {svc_title}: {len(entities)} entities")
            return entities[:5]
    # Also check _metadata_cache
    if service_path in _metadata_cache:
        entities = [e.get("entity_type", "") for e in _metadata_cache[service_path]
                    if not e.get("entity_type", "").startswith(("I_", "SAP__"))]
        if entities:
            logger.info(f"Metadata cache hit for {service_path}: {len(entities)} entities")
            return entities[:5]
    try:
        import xml.etree.ElementTree as ET
...
def _generate_server_code_with_bedrock(prompt: str, services_with_entities: list,
    ...
    system = (
        ...
        "1. The URL path is: /sap/opu/odata/sap/<SERVICE_NAME>/<ENTITY_SET_NAME>\n"
        "2. SERVICE_NAME and ENTITY_SET_NAME are DIFFERENT — NEVER repeat the service name as the entity set.\n"
        "3. Entity set names from metadata end in 'Type' — strip the suffix: "
        "A_SalesOrderType → A_SalesOrder, A_SupplierInvoiceType → A_SupplierInvoice.\n"
        "4. ALWAYS use the exact paths provided in the prompt — do not invent or guess entity set names.\n"
        "5. NEVER use the service name as the entity set name.\n\n"
        "Use ONLY this exact function signature pattern:\n"
        "@mcp.tool()\n"
        "def func_name(ctx: Context, top: int = 10, skip: int = 0, filter_expr: str = \"\", bearer_token: str = \"\"):\n"
        "    \"\"\"docstring\"\"\"\n"
        ...
        "        params = {\"$format\": \"json\", \"$top\": str(top), \"$skip\": str(skip)}\n"
        "        if filter_expr: params[\"$filter\"] = filter_expr\n"
        "        data = _sap_get(\"/sap/opu/odata/sap/<SERVICE_NAME>/<ENTITY_SET_NAME>\", token, params)\n"
        "        results = data.get(\"d\", {}).get(\"results\", [])\n"
        "        return json.dumps({\"count\": len(results), \"results\": results}, indent=2)\n"
        "    except Exception as e:\n"
        "        return json.dumps({\"error\": str(e)})\n\n"
        "Return ONLY the tool function definitions, no imports, no main block. Do NOT add return type annotations like -> str."
    )
    user = (
        f"User request: {prompt}\n\n"
        f"Use EXACTLY these service paths and entity sets — do not change them:\n"
    )
    for svc in services_with_entities:
        svc_path = svc["service_path"]
        for ent in svc["entities"][:3]:
            # Strip 'Type' suffix to get entity set name
            entity_set = ent[:-4] if ent.endswith("Type") else ent
            user += f"  _sap_get(\"{svc_path}/{entity_set}\", token, params)\n"
    user += (
        f"\nGenerate 3-7 MCP tool functions. Each function must call _sap_get with one of the exact paths above. "
        f"Use snake_case function names that describe what the tool does."
    )
...
# ── Templates for Cloud ALM and SuccessFactors generated agents ───────────────
_CALM_SERVER_TEMPLATE = '''"""
{description}
Auto-generated by AI Factory MCP Server — SAP Cloud ALM focused agent.
"""
...
def _get_calm_token() -> str:
    ...
    with httpx.Client(timeout=15.0) as c:
        r = c.post(token_url, data={{"grant_type": "client_credentials"}},
                   auth=(os.environ["CALM_CLIENT_ID"], os.environ["CALM_CLIENT_SECRET"]),
                   headers={{"Accept": "application/json"}})
        r.raise_for_status(); d = r.json()
    _token_cache["token"] = d["access_token"]
    _token_cache["expires_at"] = now + d.get("expires_in", 3600) - 300
    return _token_cache["token"]
...
def _calm_get(path, params=None):
    ...
    with httpx.Client(timeout=30.0) as c:
        r = c.get(f"{{base}}{{path}}", params=params or {{}},
                  headers={{"Authorization": f"Bearer {{_get_calm_token()}}", "Accept": "application/json"}})
...
_SF_SERVER_TEMPLATE = '''"""
{description}
Auto-generated by AI Factory MCP Server — SAP SuccessFactors focused agent.
"""
import os, json, logging, httpx, time as _time
from mcp.server.fastmcp import FastMCP, Context
...
def _get_sf_token() -> str:
    ...
    token_url = os.environ.get("SF_TOKEN_URL",
        f"https://api{{os.environ.get('SF_DC','4')}}.successfactors.com/oauth/token")
    with httpx.Client(timeout=15.0) as c:
        r = c.post(token_url, data={{"grant_type": "client_credentials",
...
def _sf_get(path, params=None):
    ...
    with httpx.Client(timeout=30.0) as c:
        r = c.get(f"{{base}}{{path}}", params={{**(params or {{}}), "$format": "json"}},
                  headers={{"Authorization": f"Bearer {{_get_sf_token()}}", "Accept": "application/json"}})
        r.raise_for_status(); return r.json()
...
def _detect_domain(prompt: str) -> str:
    """Return 'calm', 'sf', or 's4' based on prompt keywords."""
    p = prompt.lower()
...
@mcp.tool()
def propose_mcp_server(ctx: Context, prompt: str, recent_services: str = "",
                       agent_name: str = "") -> str:
    """Propose a new focused MCP server before generating and deploying it.

    IMPORTANT FOR THE LLM: Before calling this tool, review the ENTIRE conversation
    history and collect ALL SAP OData services, entity sets, Cloud ALM APIs, or
    SuccessFactors APIs that were queried during this session. Pass them as a
    comma-separated list in 'recent_services'.
    Format: "SERVICE_NAME/EntitySet, SERVICE_NAME/EntitySet"
    Example: "API_SALES_ORDER_SRV/A_SalesOrder, API_BUSINESS_PARTNER/A_BusinessPartner"

    If no services were used in the conversation, leave recent_services empty —
    the tool will discover relevant services from the SAP catalog based on the prompt.

    Args:
        prompt: What the MCP server should do (e.g. 'Sales order management with partner details')
        recent_services: Comma-separated services/entities from conversation history (LLM fills this)
        agent_name: Short identifier (e.g. 'sap_sales_order_agent'). Auto-generated if empty.
    """
    domain = _detect_domain(prompt)

    if not agent_name:
        slug = "_".join(w for w in prompt.lower().split()[:4]
                        if w not in {"the","a","an","for","with","and","or"})
        agent_name = f"sap_{slug}_agent".replace("-", "_")[:40]
    ...
    # ── Option 1: Use conversation history if provided ────────────────────
    if recent_services.strip():
        proposal["source"] = "conversation_history"
        seen_svc = set()
        seen_ent = set()
        for item in recent_services.split(","):
            item = item.strip()
            if "/" in item:
                svc, ent = item.split("/", 1)
                svc, ent = svc.strip(), ent.strip()
                if svc and svc not in seen_svc:
                    seen_svc.add(svc); proposal["services"].append(svc)
                if ent and ent not in seen_ent:
                    seen_ent.add(ent); proposal["entities"].append(ent)
            elif item:
                if item not in seen_svc:
                    seen_svc.add(item); proposal["services"].append(item)

    # ── Option 2: Cloud ALM — use known API map ──────────────────────────
    elif domain == "calm":
        proposal["source"] = "cloud_alm_api_map"
        calm_apis = {
            "Projects":          "/api/calm-projects/v1/projects",
            "Tasks":             "/api/calm-tasks/v1",
            ...
            "MonitoringEvents":  "/api/calm-processmonitoring/v1/MonitoringEvents",
            "Analytics":         "/api/calm-analytics/v1",
            "ProcessHierarchy":  "/api/calm-processhierarchy/v1/ProcessHierarchyNodes",
        }
        # Filter by prompt keywords
        p = prompt.lower()
        for name, path in calm_apis.items():
            if any(k in p for k in [name.lower(), name.lower().rstrip("s")]):
                proposal["services"].append(path)
                proposal["entities"].append(name)
        if not proposal["services"]:
            proposal["services"] = list(calm_apis.values())
            proposal["entities"] = list(calm_apis.keys())

    # ── Option 3: SuccessFactors — use known entity map ──────────────────
    elif domain == "sf":
        proposal["source"] = "successfactors_entity_map"
        sf_entities = {
            "User": "/odata/v2/User", "PerPerson": "/odata/v2/PerPerson",
            "EmpEmployment": "/odata/v2/EmpEmployment", "Position": "/odata/v2/Position",
            "FODepartment": "/odata/v2/FODepartment", "JobRequisition": "/odata/v2/JobRequisition",
            "Candidate": "/odata/v2/Candidate", "LearningActivity": "/odata/v2/LearningActivity",
            "PerformanceReview": "/odata/v2/PerformanceReview",
            "CompensationEmployee": "/odata/v2/CompensationEmployee",
        }
        p = prompt.lower()
        for name, path in sf_entities.items():
            if any(k in p for k in [name.lower()]):
                proposal["services"].append(path)
                proposal["entities"].append(name)
        if not proposal["services"]:
            proposal["services"] = list(sf_entities.values())
            proposal["entities"] = list(sf_entities.keys())

    # ── Option 4: S/4HANA — discover from catalog ────────────────────────
    else:
        proposal["source"] = "sap_catalog_discovery"
        token = _get_token(ctx)
        if token:
            stop_words = {"with","that","this","from","have","will","should","tools","tool",
                          ...
                          "create","server","agent","deploy","generate","build","make","new"}
            ...
            for svc in matched[:8]:
                svc_title = svc["Title"]
                proposal["services"].append(f"/sap/opu/odata/sap/{svc_title}")
                entities = _get_entities_for_service(f"/sap/opu/odata/sap/{svc_title}", token)
                for ent in entities:
                    proposal["entities"].append(ent.get("entity_set", ent.get("entity_type", "")))

    # ── Build enriched prompt with exact resolved paths ──────────────────
    # For S4: build full OData paths (service/entity_set)
    resolved_paths = []
    if domain == "s4":
        for i, svc in enumerate(proposal["services"][:8]):
            # Match entities to services by index
            ent = proposal["entities"][i] if i < len(proposal["entities"]) else ""
            if ent:
                ent_set = ent[:-4] if ent.endswith("Type") else ent
                resolved_paths.append(f"{svc}/{ent_set}")
    elif domain == "calm":
        resolved_paths = proposal["services"][:8]
    elif domain == "sf":
        resolved_paths = proposal["services"][:8]

    # Enriched prompt includes exact paths so generator never guesses
    enriched_prompt = prompt
    if resolved_paths:
        enriched_prompt = (
            f"{prompt}. "
            f"Use EXACTLY these API paths: {', '.join(resolved_paths)}"
        )

    return json.dumps({
        "proposal": proposal,
        "estimated_tools": max(3, min(8, len(proposal["entities"]))),
        "enriched_prompt": enriched_prompt,
        "instruction": (
            "Present this proposal to the user clearly:\n"
            f"- Agent name: {agent_name}\n"
            f"- Domain: {domain}\n"
            f"- Services: {', '.join(proposal['services'][:10])}\n"
            f"- Entities: {', '.join(proposal['entities'][:15])}\n"
            f"- Source: {proposal['source']}\n\n"
            "Ask the user: 'Should I go ahead and generate + deploy this MCP server?'\n"
            f"If user confirms, call generator_agent_tool with this EXACT question:\n"
            f"'Deploy a new MCP server. Agent name: {agent_name}. "
            f"Prompt: {enriched_prompt}. Call generate_and_deploy_mcp_server NOW.'\n"
            "If user wants changes, adjust and call propose_mcp_server again."
        )
    }, indent=2)
...
def generate_and_deploy_mcp_server(ctx: Context, prompt: str, agent_name: str) -> str:
    ...
    Automatically detects the domain from the prompt:
    - SAP S/4HANA OData (default) — discovers OData services from the catalog
    - SAP Cloud ALM — generates Cloud ALM REST/OData tools (projects, tasks, monitoring, analytics…)
    - SAP SuccessFactors — generates SF OData tools (Employee Central, Recruiting, Learning…)
    ...
    """
    token = _get_token(ctx)
    # Token is optional — only needed for S4 catalog discovery when services aren't in the prompt
    ...
    try:
        ...
        if domain == "calm":
            ...
            system = (
                ...
                "for a FastMCP server that calls SAP Cloud ALM REST/OData APIs.\n"
                "Use _calm_get(path, params) for GET and _calm_post(path, body) for POST.\n"
                "OData params: {'$top': n, '$skip': n, '$filter': '...', '$select': '...'}\n"
                "Each function: def func_name(ctx: Context, ...)\n"
                "Return ONLY function definitions, no imports, no main block. Do NOT add return type annotations like -> str."
            )
            user = (
                f"User request: {prompt}\n\n"
                f"Available Cloud ALM API paths:\n{json.dumps(calm_apis, indent=2)}\n\n"
                f"Generate 4-8 focused MCP tool functions for this request. "
                f"Use descriptive snake_case names like calm_list_monitoring_events, calm_get_project_tasks."
            )
            ...
            generated_tools = json.loads(resp["body"].read())["content"][0]["text"]
            server_code = _CALM_SERVER_TEMPLATE.format(
                description=f"SAP Cloud ALM MCP Server for: {prompt}",
                agent_name=agent_name, tools=generated_tools)
            services_used = list(calm_apis.values())

        # ── SuccessFactors domain ─────────────────────────────────────────────
        elif domain == "sf":
            sf_apis = {
                "EmployeeTime":         "/odata/v2/EmployeeTime",
                "User":                 "/odata/v2/User",
                "EmpEmployment":        "/odata/v2/EmpEmployment",
                "PerPerson":            "/odata/v2/PerPerson",
                "PerPersonal":          "/odata/v2/PerPersonal",
                "JobRequisition":       "/odata/v2/JobRequisition",
                "Candidate":            "/odata/v2/Candidate",
                "LearningActivity":     "/odata/v2/LearningActivity",
                "PerformanceReview":    "/odata/v2/PerformanceReview",
                "CompensationEmployee": "/odata/v2/CompensationEmployee",
                "ECEmploymentInfo":     "/odata/v2/ECEmploymentInfo",
                "Position":             "/odata/v2/Position",
                "FODepartment":         "/odata/v2/FODepartment",
                "FOLocation":           "/odata/v2/FOLocation",
            }
            bedrock = boto3.client("bedrock-runtime", region_name=region)
            system = (
                "You are an expert SAP SuccessFactors developer. Generate Python MCP tool functions "
                "for a FastMCP server that calls SAP SuccessFactors OData v2 APIs.\n"
                "Use _sf_get(path, params) for all reads. OData params: {'$top': n, '$skip': n, '$filter': '...', '$select': '...'}\n"
                "Each function: def func_name(ctx: Context, top: int = 10, skip: int = 0, filter_expr: str = '')\n"
                "Parse results from response.get('d', {}).get('results', [])\n"
                "Return ONLY function definitions, no imports, no main block. Do NOT add return type annotations like -> str."
            )
            user = (
                f"User request: {prompt}\n\n"
                f"Available SuccessFactors OData entity sets:\n{json.dumps(sf_apis, indent=2)}\n\n"
                f"Generate 4-8 focused MCP tool functions for this request. "
                f"Use descriptive snake_case names like sf_list_employees, sf_get_job_requisitions."
            )
            resp = bedrock.invoke_model(
                modelId="us.anthropic.claude-sonnet-4-20250514-v1:0",
                body=json.dumps({"anthropic_version": "bedrock-2023-05-31", "max_tokens": 4096,
                ...
                contentType="application/json", accept="application/json")
            generated_tools = json.loads(resp["body"].read())["content"][0]["text"]
            server_code = _SF_SERVER_TEMPLATE.format(
                description=f"SAP SuccessFactors MCP Server for: {prompt}",
                agent_name=agent_name, tools=generated_tools)
            services_used = list(sf_apis.values())

        # ── SAP S/4HANA OData (default) ───────────────────────────────────────
        else:
            # First: extract API service names directly from the prompt (e.g. API_PURCHASEORDER_PROCESS_SRV)
            import re
            explicit_services = re.findall(r'(API_[A-Z0-9_]+_SRV)', prompt.upper())
            if explicit_services:
                logger.info(f"Extracted services from prompt: {explicit_services}")
                services_with_entities = []
                for svc_name in explicit_services[:5]:
                    svc_path = f"/sap/opu/odata/sap/{svc_name}"
                    # Fetch real entity sets from cache or SAP $metadata — never guess
                    entities = _get_entities_for_service(svc_path, token or "")
                    if not entities:
                        logger.warning(f"Could not resolve entity sets for {svc_name} — skipping")
                        continue
                    services_with_entities.append({"service_path": svc_path,
                                                   "title": svc_name, "entities": entities})
            else:
                # Fallback: discover from catalog (needs SAP token)
                if not token:
                    return json.dumps({"error": "No SAP bearer token and no API service names found in prompt. "
                                       "Include service names like API_SALES_ORDER_SRV in your prompt, or ensure SAP token is available."})
                stop_words = {"with", "that", "this", "from", "have", "will", "should", "tools", "tool",
                              "details", "including", "information", "related", "help", "about", "also",
                              "their", "them", "these", "those", "into", "like", "such", "some", "other",
                              "more", "most", "very", "just", "only", "also", "both", "each", "every"}
            ...
            if not services_with_entities:
                return json.dumps({"error": "Could not resolve any services. Include API service names in your prompt."})
        ...
        # ── Common: requirements, meta, S3 upload, CodeBuild ─────────────────
        requirements = "mcp>=1.10.0,<1.20.0\nhttpx\nbedrock-agentcore-starter-toolkit\nboto3\n"
        meta = {"agent_name": agent_name, "prompt": prompt, "domain": domain,
...
# ── SAP Cloud ALM ITSM ───────────────────────────────────────────────────────
# Auth: OAuth 2.0 client credentials flow against SAP BTP XSUAA
# Env vars required: CALM_TOKEN_ENDPOINT, CALM_API, CALM_CLIENT_ID, CALM_CLIENT_SECRET, CALM_REPORTER

import time as _time

_calm_token_cache: dict = {"token": None, "expires_at": 0}

def _get_calm_token() -> str:
    """Get (or refresh) a Cloud ALM OAuth2 client-credentials token."""
    now = _time.time()
    if _calm_token_cache["token"] and now < _calm_token_cache["expires_at"]:
        return _calm_token_cache["token"]
    ...
    if not all([endpoint, client_id, client_secret]):
        raise ValueError("CALM_TOKEN_ENDPOINT, CALM_CLIENT_ID and CALM_CLIENT_SECRET env vars are required")
    with httpx.Client(timeout=15.0) as c:
        r = c.post(
        ...
        r.raise_for_status()
        data = r.json()
    _calm_token_cache["token"] = data["access_token"]
    _calm_token_cache["expires_at"] = now + data.get("expires_in", 3600) - 300  # 5-min buffer
    return _calm_token_cache["token"]

def _calm_get(path: str, params: dict = None) -> dict:
    base = os.environ.get("CALM_API", "").rstrip("/")
    token = _get_calm_token()
    with httpx.Client(timeout=30.0) as c:
        r = c.get(f"{base}{path}", params=params or {},
...
@mcp.tool()
def calm_get_installations(offset: int = 0, limit: int = 50, search_text: str = "") -> str:
    """Get SAP Cloud ALM landscape installations (systems/tenants) with optional pagination and search.
    Returns count, totalCount and a results array with fields: customerNbr, customerName,
    productTxt, systemName, systemType, installationNbr, systemId, cloudURL, isCloud, etc.
    Use search_text to do a full-text search across system name, product, customer, etc.
    """
    reporter = os.environ.get("CALM_REPORTER", "")
    params: dict = {"offset": offset, "limit": limit, "reporter": reporter}
    if search_text:
        params["searchText"] = search_text
        params["onlyCaseSelectable"] = "true"
    try:
        data = _calm_get("/api/calm-itsmapi/v1/supportcases/masterdata/landscapeObjects", params)
...
@mcp.tool()
def calm_filter_installations(system_type: str = "", customer_nbr: str = "",
                               product_txt: str = "", offset: int = 0, limit: int = 50) -> str:
    """Filter SAP Cloud ALM installations by system_type (e.g. BTP, Public Cloud),
    customer_nbr, or product_txt (partial match). At least one filter should be provided.
    Returns matching installations from the landscape.
    """
    reporter = os.environ.get("CALM_REPORTER", "")
    params: dict = {"offset": offset, "limit": limit, "reporter": reporter}
    try:
        ...
        if system_type:
            results = [r for r in results if r.get("systemType", "") == system_type]
        if customer_nbr:
            results = [r for r in results if r.get("customerNbr", "") == customer_nbr]
        if product_txt:
            pt = product_txt.lower()
            results = [r for r in results if pt in r.get("productTxt", "").lower()]
        return json.dumps({"count": len(results), "results": results}, indent=2)
    except Exception as e:
        return json.dumps({"error": str(e)})
...
@mcp.tool()
def calm_get_support_components(offset: int = 0, limit: int = 50) -> str:
    """Get the list of SAP support components available in Cloud ALM ITSM.
    Useful for understanding what support areas/components are configured.
    """
    try:
        data = _calm_get("/api/calm-itsmapi/v1/supportcases/masterdata/components",
...
@mcp.tool()
def calm_get_contacts(customer_nbr: str, offset: int = 0, limit: int = 50) -> str:
    """Get contacts for a specific customer from SAP Cloud ALM ITSM.
    Requires customer_nbr (e.g. '0001801161'). Returns contact persons associated
    with that customer in the Cloud ALM landscape.
    """
    reporter = os.environ.get("CALM_REPORTER", "")
    if not reporter:
        return json.dumps({"error": "CALM_REPORTER env var is required for this endpoint"})
    try:
        data = _calm_get("/api/calm-itsmapi/v1/supportcases/masterdata/contacts",
                         {"offset": offset, "limit": limit,
                          "reporter": reporter, "customerNumber": customer_nbr})
        return json.dumps(data, indent=2)
    except Exception as e:
        return json.dumps({"error": str(e)})


# ── SAP Cloud ALM OData/REST (Projects, Tasks, Features, Documents, Test Mgmt,
#    Process Hierarchy, Process Monitoring, Analytics) ─────────────────────────
from calm_tools import (
    _calm_odata_token, _calm_base_url, _calm_headers, _calm_req, _odata_params,
...
# Register all Cloud ALM tools with FastMCP
mcp.tool()(calm_list_projects)
mcp.tool()(calm_get_project)
mcp.tool()(calm_list_project_timeboxes)
mcp.tool()(calm_list_project_teams)
mcp.tool()(calm_list_programs)
mcp.tool()(calm_create_project)
mcp.tool()(calm_list_tasks)
mcp.tool()(calm_get_task)
mcp.tool()(calm_create_task)
mcp.tool()(calm_update_task)
mcp.tool()(calm_delete_task)
mcp.tool()(calm_list_task_comments)
mcp.tool()(calm_create_task_comment)
mcp.tool()(calm_list_workstreams)
mcp.tool()(calm_list_features)
mcp.tool()(calm_get_feature)
mcp.tool()(calm_create_feature)
mcp.tool()(calm_update_feature)
mcp.tool()(calm_delete_feature)
mcp.tool()(calm_list_feature_statuses)
mcp.tool()(calm_list_feature_priorities)
mcp.tool()(calm_list_documents)
mcp.tool()(calm_get_document)
mcp.tool()(calm_create_document)
mcp.tool()(calm_update_document)
mcp.tool()(calm_delete_document)
mcp.tool()(calm_list_document_types)
mcp.tool()(calm_list_testcases)
mcp.tool()(calm_get_testcase)
mcp.tool()(calm_create_testcase)
mcp.tool()(calm_list_test_activities)
mcp.tool()(calm_list_test_actions)
mcp.tool()(calm_list_hierarchy_nodes)
mcp.tool()(calm_get_hierarchy_node)
mcp.tool()(calm_create_hierarchy_node)
mcp.tool()(calm_update_hierarchy_node)
mcp.tool()(calm_delete_hierarchy_node)
mcp.tool()(calm_list_monitoring_events)
mcp.tool()(calm_get_monitoring_event)
mcp.tool()(calm_list_monitored_services)
mcp.tool()(calm_list_analytics_providers)
mcp.tool()(calm_query_analytics)
# SuccessFactors
mcp.tool()(sf_list_employees)
mcp.tool()(sf_get_employee_employment)
mcp.tool()(sf_list_positions)
mcp.tool()(sf_list_departments)
mcp.tool()(sf_list_locations)
mcp.tool()(sf_list_job_requisitions)
mcp.tool()(sf_list_candidates)
mcp.tool()(sf_list_learning_activities)
mcp.tool()(sf_list_performance_reviews)
mcp.tool()(sf_list_compensation)
mcp.tool()(sf_list_users)


# ── AI Factory: 5 Strands Router Tools ───────────────────────────────────────
# These are the PRIMARY tools exposed to Kiro. Each one is a Strands agent
# that routes to the right internal tools based on the question.
# The 92 individual tools above are still available but these 5 are the
# recommended entry points for natural language interaction.

def _run_strands(system_prompt: str, question: str, tool_functions: list, token: str) -> str:
    """Run a Strands agent with direct Python function tools (no MCP loopback needed)."""
    from strands import Agent
    from strands.models import BedrockModel

    MODEL_ID = "us.anthropic.claude-sonnet-4-6"

    # Inject token into env so tools can pick it up via SAP_BEARER_TOKEN fallback
    if token:
    ...
    try:
        ...
        # Extract plain text from AgentResult — avoids MCP outputSchema validation failure
        # when the framework tries to match a stringified AgentResult object against the schema
        ...
        logger.error(f"Strands router error: {e}")
        return json.dumps({"error": str(e)})


_ADT_TOOLS = [
    call_adt_api, upload_abap_source, get_abap_program, get_abap_class,
    get_function_module, get_abap_interface, get_abap_include, search_objects,
    get_package, get_transaction, get_table_definition, get_table_contents,
    get_type_info, syntax_check, run_atc_check, create_transport,
    release_transport, list_user_transports, create_odata_service,
    activate_odata_service, list_backend_services, adt_discovery,
]
_ODATA_TOOLS = [
    discover_sap_services, search_sap_services, get_service_metadata,
    query_sap_odata, get_sap_entity, update_sap_entity, create_sap_entity,
    get_service_entities, get_entity_properties, find_entity_by_field,
    cache_stats, smart_query,
]
_CALM_TOOLS = [
    calm_get_installations, calm_filter_installations, calm_get_support_components,
    calm_get_contacts, calm_list_projects, calm_get_project,
    calm_list_project_timeboxes, calm_list_project_teams, calm_list_programs,
    calm_create_project, calm_list_tasks, calm_get_task, calm_create_task,
    calm_update_task, calm_delete_task, calm_list_task_comments,
    calm_create_task_comment, calm_list_workstreams, calm_list_features,
    calm_get_feature, calm_create_feature, calm_update_feature, calm_delete_feature,
    calm_list_feature_statuses, calm_list_feature_priorities, calm_list_documents,
    calm_get_document, calm_create_document, calm_update_document,
    calm_delete_document, calm_list_document_types, calm_list_testcases,
    calm_get_testcase, calm_create_testcase, calm_list_test_activities,
    calm_list_test_actions, calm_list_hierarchy_nodes, calm_get_hierarchy_node,
    calm_create_hierarchy_node, calm_update_hierarchy_node, calm_delete_hierarchy_node,
    calm_list_monitoring_events, calm_get_monitoring_event, calm_list_monitored_services,
    calm_list_analytics_providers, calm_query_analytics,
]
_SF_TOOLS = [
    sf_list_employees, sf_get_employee_employment, sf_list_positions,
    sf_list_departments, sf_list_locations, sf_list_job_requisitions,
    sf_list_candidates, sf_list_learning_activities, sf_list_performance_reviews,
    sf_list_compensation, sf_list_users,
]
_GEN_TOOLS = [generate_and_deploy_mcp_server]


@mcp.tool()
def adt_agent_tool(ctx: Context, question: str):
    """AI Factory — ADT Agent for ABAP development and SAP object exploration.

    Use for: reading/writing ABAP source code, syntax checks, ATC quality checks,
    transport management, DDIC table definitions, creating OData via CDS views,
    activating OData services, exploring SAP repository objects.
    Always ask before creating or deploying anything.
    """
    return _run_strands(
        ...
        question, _ADT_TOOLS, _get_token(ctx))


@mcp.tool()
def odata_agent_tool(ctx: Context, question: str):
    """AI Factory — OData Agent for SAP S/4HANA business data retrieval.

    FIRST tool to use for any S/4HANA data query: sales orders, purchase orders,
    invoices, RFQs, materials, vendors, customers, stock, deliveries.
    Searches OData catalog, queries entity sets, handles natural language questions.
    If OData not active → tries to activate. If no OData → use adt_agent_tool for SQL.
    """
    return _run_strands(
        ...
        question, _ODATA_TOOLS, _get_token(ctx))


@mcp.tool()
def calm_agent_tool(ctx: Context, question: str):
    """AI Factory — Cloud ALM Agent for SAP Rise / Cloud ERP customers.

    Use for SAP Cloud ALM: implementation projects, tasks, features, documents,
    test management, process hierarchy, process monitoring events, analytics,
    ITSM support cases, landscape installations.
    Customer must be on SAP Rise / Cloud ERP offering.
    """
    return _run_strands(
        "You are the Cloud ALM Agent — SAP Rise/Cloud ERP specialist. "
        "For analytics, call calm_list_analytics_providers first, then calm_query_analytics.",
        question, _CALM_TOOLS, _get_token(ctx))


@mcp.tool()
def sf_agent_tool(ctx: Context, question: str):
    """AI Factory — SuccessFactors Agent for SAP HCM and HR use cases.

    Use for: employee data, positions, departments, recruiting (job requisitions,
    candidates), learning activities, performance reviews, compensation.
    Always filter results when a specific employee or department is mentioned.
    """
    return _run_strands(
        "You are the SuccessFactors Agent — SAP HCM specialist. "
        "Always use filter_ to narrow results. Avoid large unfiltered datasets.",
        question, _SF_TOOLS, _get_token(ctx))


@mcp.tool()
def generator_agent_tool(ctx: Context, question: str):
    ...
    """
    return _run_strands(
        "You are the Generator Agent. Use generate_and_deploy_mcp_server. "
        "Always confirm agent_name and domain before deploying.",
        question, _GEN_TOOLS, _get_token(ctx))
