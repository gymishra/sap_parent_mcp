...
def _load_cached_token():
    ...
    cache_paths = [
        os.path.join(script_dir, ".okta_token_cache.json"),
        os.path.join(os.getcwd(), "sap-smart-agent", ".okta_token_cache.json"),
        os.path.join(os.getcwd(), ".okta_token_cache.json"),
    ]
...
def get_okta_token():
    ...
    if not CallbackHandler.auth_code:
        raise RuntimeError(f"Okta login timed out. Please open this URL manually and try again:\n{url}")
...
def get_mcp_url():
    local = os.environ.get("LOCAL_MCP_URL", "")
    if local:
        return local
    s = Session()
...
def _strip_output_schema(tools):
    """Remove outputSchema from all tools — prevents Kiro validation errors."""
    for tool in tools:
...
server = Server("sap_credit_memo_agent-bridge")
